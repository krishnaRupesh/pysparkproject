import json

import boto3
from botocore.exceptions import ClientError

s3 = boto3.client('s3')
# kinesis_client = boto3.client('kinesis')
kinesis_client = boto3.client('kinesis', region_name='eu-central-1',
                      aws_access_key_id="ASIAX67SXGJKYQBTQRMK",
                      aws_secret_access_key="pdWG3+TaE8wcajYMMRLzh5BEX4IVuOJJjFM712gP",
                      aws_session_token=""
                              )

f = open(r"C:\Users\krishnan\Downloads\billing_kinesis.csv", "r")
lines = f.readlines()
lines = [json.dumps(json.loads(msg)) for msg in lines]

if lines:
    put_records = [{'Data': msg, 'PartitionKey': str('billing')} for msg in lines]
    try:
        kinesis_client.put_records(Records=put_records, StreamName='Peza-reporting-events')
    except ClientError:
        print("putRecords failed for " )
        print("number of records in file is" + str(len(lines)))
        raise
